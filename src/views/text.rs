...
fn turn_enc_into_dec(password: &str, enc_text: &str) -> String {
    let text = hex::decode(enc_text).map_err(|e| e.to_string());
    if let Err(err) = text {
    ...
    use_effect(move || match *last_touched.read() {
        LastTouched::Encrypted => {
            if encrypted_text().is_empty() {
                ...
                decrypted_text.set(turn_enc_into_dec(&password(), &encrypted_text()));
            }
        }
        LastTouched::Decrypted => {
            if decrypted_text().is_empty() {
                ...
                encrypted_text.set(turn_dec_into_enc(&password(), mode(), &decrypted_text()));
            }
        }
    });
